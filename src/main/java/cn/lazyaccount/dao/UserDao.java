...
public class UserDao {
    public User findByUsername(String username) {//根据用户名查询用户
        ...
        return BaseDao.queryForObject(sql, new BaseDao.RowMapper<User>() {// 使用BaseDao执行查询并映射结果
            @Override
            public User mapRow(ResultSet rs) throws SQLException {//结果集映射方法
                ...
                user.setNickname(rs.getString("nickname"));//设置昵称
                user.setCreateTime(rs.getTimestamp("create_time"));//设置创建时间
                return user; // 返回用户对象
            }
        }, username); // username是SQL参数
    }
    public boolean addUser(User user) {//添加新用户
        String sql = "INSERT INTO t_user (username, password, nickname) VALUES (?, ?, ?)";//SQL插入语句
        int result = BaseDao.executeUpdate(sql,// 执行插入操作，返回受影响的行数
                user.getUsername(), // 第一个参数：用户名
                user.getPassword(), // 第二个参数：密码
                user.getNickname() != null ? user.getNickname() : user.getUsername() // 第三个参数：昵称（昵称为空，则使用用户名作为昵称）
        );
        return result > 0;// 受影响行数大于0表示成功
    }
    public User login(String username, String password) {//用户登录验证
    ...
    public boolean isUsernameExists(String username) {//检查用户名是否已存在
        ...
        Integer count = BaseDao.queryForObject(sql, new BaseDao.RowMapper<Integer>() {// 执行查询，返回计数
            @Override
            public Integer mapRow(ResultSet rs) throws SQLException {//结果集映射方法
    ...
    public boolean updateUser(User user) {//更新用户信息
        String sql = "UPDATE t_user SET nickname = ? WHERE id = ?";// SQL更新语句，只更新昵称字段
        int result = BaseDao.executeUpdate(sql,// 执行更新操作
                user.getNickname(),//新昵称
                user.getId()//用户ID（WHERE条件）
        );
